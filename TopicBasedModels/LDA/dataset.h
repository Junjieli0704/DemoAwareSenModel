...
class document {
public:
    int * words;          //all word_ids in the document
    string docID;
    int length;
	...
	bool is_contain_word(int temp_word){
	    if (length == 0) return false;
	    for (int i = 0; i < length; i++){
            if (temp_word == this->words[i]) return true;
	    }
	    return false;
	}
...
public:
    document ** docs;               // store training data vocab ID
                                    // The reasons for two pointers is: -> plus docs[]
    ...
    */
    ...
    int read_data(string dfile, string wordmapfile);
